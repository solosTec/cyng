...
namespace cyng {
	namespace net {
		...
		public:
			...
			channel_ptr create_channel(typename S::endpoint_type ep, std::function<void(boost::system::error_code)> cb_accept) {
				//
				//	create an uuid
				//
				...
				return ctl_.create_named_channel_with_ref<server_t>(tag, ctx, ep, cb_accept);
				//return channel_ptr();
			}
		private:
			controller& ctl_;
