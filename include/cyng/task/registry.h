...
namespace cyng {
    ...
    class registry {
      ...
      public:
         ...
         */
        void dispatch(std::vector<std::size_t> tasks, std::string slot, channel::cb_err_t, tuple_t &&msg);
         ...
         */
        void dispatch(std::string channel, std::string slot, channel::cb_err_t cb, tuple_t msg);
         ...
         * @msg data to dispatch. Data should be match with the function signature.
         */
        void dispatch(std::size_t channel, std::string slot, channel::cb_err_t cb, tuple_t msg);

        /**
         * convinience function to dispatch(std::string channel, std::string slot, tuple_t&& msg);
         */
        template <typename... Args> void dispatch(std::string channel, std::string slot, channel::cb_err_t cb, Args &&...args) {
            dispatch(channel, slot, cb, cyng::make_tuple(std::forward<Args>(args)...));
        }
         ...
         */
        void dispatch_exclude(std::size_t id, std::string channel, std::string slot, channel::cb_err_t cb, tuple_t &&msg);
         ...
         * @return Number of channels found
         */
        void dispatch_exclude(channel_ptr channel, std::string slot, channel::cb_err_t cb, tuple_t &&msg);
