...
namespace cyng {
    ...
    template <std::size_t N> std::ostream &operator<<(std::ostream &os, digest<N> const &d) {
    ...
    template <std::size_t N> std::ostream &operator<<(std::ostream &os, aes_key<N> const &key) {
     ...
     */
    template <typename R, typename P> std::ostream &operator<<(std::ostream &os, std::chrono::duration<R, P> const &v) {
        ...
        os << std::setfill('0') << std::setw(2) << h.count() << ':' << std::setw(2) << (m.count() % 60ULL) //	minutes
           << ':'
    ...
    //		//	std::time_t const tt = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now() +
    //std::chrono::duration_cast<std::chrono::system_clock::duration>(tp - std::chrono::steady_clock::now())); 		auto tm =
    //*std::localtime(&tt); 		return os << std::put_time(&tm, "%Y-%m-%dT%H:%M:%S%z");
    //	}
    ...
    template <typename T> std::ostream &operator<<(std::ostream &os, color<T> const &v) {
        boost::io::ios_flags_saver ifs(os);

        os << "#" << std::setfill('0') << std::hex << std::setw(2 * sizeof(T)) << +v.red() << std::setw(2 * sizeof(T)) << +v.green()
           << std::setw(2 * sizeof(T)) << +v.blue();
        return os;
    }
     ...
     */
    template <typename Head, typename... Args> std::string to_path(char delim, Head &&v, Args &&...args) {
        std::stringstream ss;
        ss << v;
        ((ss << delim << args), ...);
        return ss.str();
