...
namespace cyng {
    namespace io {
         ...
         * fallback
         */
        template <typename T> struct serializer<T, JSON> {
        ...
        template <> struct serializer<bool, JSON> {
            static std::size_t write(std::ostream &os, bool v);
        };

        template <> struct serializer<null, JSON> {
            static std::size_t write(std::ostream &os, null);
        };

        template <> struct serializer<vector_t, JSON> {
            static std::size_t write(std::ostream &os, vector_t const &);
        };

        template <> struct serializer<prg_t, JSON> {
            static std::size_t write(std::ostream &os, prg_t const &);
        };

        template <> struct serializer<tuple_t, JSON> {
            static std::size_t write(std::ostream &os, tuple_t const &);
        };

        template <> struct serializer<param_t, JSON> {
            static std::size_t write(std::ostream &os, param_t const &);
        };

        template <> struct serializer<param_map_t, JSON> {
            static std::size_t write(std::ostream &os, param_map_t const &);
        };

        template <> struct serializer<prop_t, JSON> {
            static std::size_t write(std::ostream &os, prop_t const &);
        };

        template <> struct serializer<prop_map_t, JSON> {
            static std::size_t write(std::ostream &os, prop_map_t const &);
        };

        template <> struct serializer<std::int8_t, JSON> {
            static std::size_t write(std::ostream &os, std::int8_t v);
        };

        template <> struct serializer<std::int16_t, JSON> {
            static std::size_t write(std::ostream &os, std::int16_t v);
        };

        template <> struct serializer<std::int32_t, JSON> {
            static std::size_t write(std::ostream &os, std::int32_t v);
        };

        template <> struct serializer<std::int64_t, JSON> {
            static std::size_t write(std::ostream &os, std::int64_t v);
        };

        template <> struct serializer<std::uint8_t, JSON> {
            static std::size_t write(std::ostream &os, std::uint8_t v);
        };

        template <> struct serializer<std::uint16_t, JSON> {
            static std::size_t write(std::ostream &os, std::uint16_t v);
        };

        template <> struct serializer<std::uint32_t, JSON> {
            static std::size_t write(std::ostream &os, std::uint32_t v);
        };

        template <> struct serializer<std::uint64_t, JSON> {
            static std::size_t write(std::ostream &os, std::uint64_t v);
        };

        template <> struct serializer<double, JSON> {
            static std::size_t write(std::ostream &os, double v);
        };

        template <> struct serializer<std::string, JSON> {
            static std::size_t write(std::ostream &os, std::string const &str);
        };

#if __cplusplus >= 202002L
        template <> struct serializer<std::u8string, JSON> {
            static std::size_t write(std::ostream &os, std::u8string const &str);
        };
#endif

        template <> struct serializer<std::u16string, JSON> {
            static std::size_t write(std::ostream &os, std::u16string const &str);
        };

        template <> struct serializer<std::u32string, JSON> {
            static std::size_t write(std::ostream &os, std::u32string const &str);
        };

        template <> struct serializer<buffer_t, JSON> {
         ...
         */
        template <> struct serializer<std::chrono::system_clock::time_point, JSON> {
         ...
         */
        template <> struct serializer<std::filesystem::path, JSON> {
         ...
         */
        template <> struct serializer<severity, JSON> {
