...
namespace cyng {
    namespace io {
        ...
        template <> struct serialize<XML> {
            template <typename T> static bool write_impl(pugi::xml_node node, object const &obj) {
                //
                //	select a serializer
                //
                using serial_t = serializer<T, XML>;
                auto ptr = object_cast<T>(obj);
                return (ptr != nullptr) ? serial_t::write(node, *ptr) : 0;
            }

            static bool write(pugi::xml_node node, object const &obj);
        };

        template <> struct serializer<null, XML> {
            static bool write(pugi::xml_node node, null);
        };

        template <> struct serializer<eod, XML> {
            static bool write(pugi::xml_node node, eod);
        };

        template <> struct serializer<bool, XML> {
            static bool write(pugi::xml_node node, bool v);
        };

        template <> struct serializer<buffer_t, XML> {
            static bool write(pugi::xml_node node, buffer_t const &v);
        };

        template <> struct serializer<std::string, XML> {
        ...
        template <> struct serializer<std::chrono::system_clock::time_point, XML> {
            static bool write(pugi::xml_node node, std::chrono::system_clock::time_point const &);
        };

        template <> struct serializer<std::filesystem::path, XML> {
            static bool write(pugi::xml_node node, std::filesystem::path const &);
        };

        template <> struct serializer<boost::asio::ip::address, XML> {
         ...
         */
        template <typename R, typename P> struct serializer<std::chrono::duration<R, P>, XML> {
        ...
        template <std::size_t N> struct serializer<aes_key<N>, XML> {
        ...
        template <> struct serializer<date, XML> {
            static bool write(pugi::xml_node node, date const &);
        };

        template <typename T> struct serializer<color<T>, XML> {
            using type = color<T>;
