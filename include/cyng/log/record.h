...
namespace cyng {
    ...
    namespace logging { //	don't collide with store records
        struct record : public std::enable_shared_from_this<record> {
            ...
            /**
             * The overloaded stream operator simplifies the use of the logger class.
             *
             * @code
             ...
             * @endcode
             */
            template <typename T> friend record &operator<<(record &rec, T const &v) {
                using cyng::operator<<;
        ...
        template <typename T> entry operator<<(entry rec, T const &v) {
            *rec << v;
            return rec;
        }
...
#define CYNG_LOG_TRACE(l, m) CYNG_LOG(l, cyng::severity::LEVEL_TRACE, m)

#ifdef _DEBUG
#define CYNG_LOG_DEBUG(l, m) CYNG_LOG(l, cyng::severity::LEVEL_DEBUG, m)
#else
#define CYNG_LOG_DEBUG(l, m) boost::ignore_unused(l);
#endif

#define CYNG_LOG_INFO(l, m) CYNG_LOG(l, cyng::severity::LEVEL_INFO, m)

#define CYNG_LOG_WARNING(l, m) CYNG_LOG(l, cyng::severity::LEVEL_WARNING, m)

#define CYNG_LOG_ERROR(l, m) CYNG_LOG(l, cyng::severity::LEVEL_ERROR, m)

#define CYNG_LOG_FATAL(l, m) CYNG_LOG(l, cyng::severity::LEVEL_FATAL, m)
