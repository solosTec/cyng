...
namespace cyng {
    ...
    object restore(std::string const &val, std::uint16_t code) {
        ...
        case TC_BOOL:
            ...
            // case TC_CHAR:
        case TC_FLOAT: return make_object(std::stof(val));
        case TC_DOUBLE: return make_object(std::stod(val));
        case TC_FLOAT80: return make_object(std::stold(val));
        case TC_UINT8:
            return (val.empty()) ? make_object(static_cast<std::uint8_t>(0u))
                                 : make_object(static_cast<std::uint8_t>(std::stoul(val, nullptr, 16)));
        case TC_UINT16:
            return (val.empty()) ? make_object(static_cast<std::uint16_t>(0u))
                                 : make_object(static_cast<std::uint16_t>(std::stoul(val, nullptr, 16)));
        case TC_UINT32:
            return (val.empty()) ? make_object(static_cast<std::uint32_t>(0u))
                                 : make_object(static_cast<std::uint32_t>(std::stoul(val, nullptr, 16)));
        case TC_UINT64:
            return (val.empty()) ? make_object(static_cast<std::uint64_t>(0u))
                                 : make_object(static_cast<std::uint64_t>(std::stoull(val, nullptr, 16)));
        case TC_INT8:
            return (val.empty()) ? make_object(static_cast<std::int8_t>(0)) : make_object(static_cast<std::int8_t>(std::stoi(val)));
        case TC_INT16:
            return (val.empty()) ? make_object(static_cast<std::int16_t>(0))
                                 : make_object(static_cast<std::int16_t>(std::stoi(val)));
        case TC_INT32:
            return (val.empty()) ? make_object(static_cast<std::int32_t>(0))
                                 : make_object(static_cast<std::int32_t>(std::stoi(val)));
        case TC_INT64:
            return (val.empty()) ? make_object(static_cast<std::int64_t>(0))
                                 : make_object(static_cast<std::int64_t>(std::stoi(val)));
        // case TC_STRING:	//	default
        case TC_FS_PATH:
            return make_object(std::filesystem::path(val));
            // case TC_TIME_POINT:
        ...
        // case TC_LABEL:
        case TC_SEVERITY:
            return make_object(to_severity(val));
            //		case TC_BUFFER:			return make_object(to_buffer(val)); //	Fixme: convert hex string to buffer
        case TC_BUFFER: BOOST_ASSERT_MSG(val.size() % 2 == 0, "invalid hex string"); return make_object(hex_to_buffer(val));
