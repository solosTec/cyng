	...
	namespace db	
	{
		namespace sqlite	
		{
			...
			{	
				...
				/**
				 * busy handler: The busy callback should not take any actions which modify 
				 * the database connection that invoked the busy handler. In other words, 
				 * the busy handler is not reentrant. Any such actions result in undefined behavior.
				 *
				 ...
				 * progress
				 */
				int progress_impl(void* p)
				{
					connection::progress_handler* h = static_cast<connection::progress_handler*>(p);
					return (*h)();
				}
...
//#endif
			...
			void connection::set_progress_handler(progress_handler h, int n) {
				progress_handler_.swap(h);
				::sqlite3_progress_handler(dbp_, n, progress_handler_ ? progress_impl : 0, &progress_handler_);
			}
