...
namespace cyng {
    namespace db {
        ...
        std::pair<std::string, bool> session::connect(param_map_t const &config) {
            return (!connection_) ? std::make_pair(std::string("unknown connection type"), false) : connection_->connect(config);
        }
        ...
        bool session::is_alive() const { return (connection_) ? connection_->is_alive() : false; }

        bool session::close() { return (connection_) ? connection_->close() : false; }

        bool session::begin() { return (connection_) ? connection_->begin() : false; }

        bool session::commit() { return (connection_) ? connection_->commit() : false; }

        bool session::rollback() { return (connection_) ? connection_->rollback() : false; }

        bool session::is_transaction() const { return (connection_) ? connection_->is_transaction() : false; }

        boost::system::error_code session::last_error() const { return connection_->last_error(); }

        statement_ptr session::create_statement() { return connection_->create_statement(); }

        bool session::execute(std::string const &sql) { return connection_->execute(sql); }
        ...
        cyng::sql::dialect session::get_dialect() const {
            return (connection_) ? connection_->get_dialect() : cyng::sql::dialect::GENERIC;
        }
        ...
        void transaction::rollback() { abort_ = true; }

        session create_db_session(param_map_t const &pmap) {
            auto const ct = to_connection_type(find_value(pmap, std::string("connection.type"), std::string("SQLite")));
            session s(ct);
            auto r = s.connect(pmap);
            if (!r.second) {
#ifdef _DEBUG_DB
                std::cerr << "**error: cannot open connection " << r.first << std::endl;
#endif
            }
            return s;
        }
