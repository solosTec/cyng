...
namespace cyng {
    namespace io {
        std::size_t serializer<bool, TYPED>::write(std::ostream &os, bool v) { return serializer<bool, PLAIN>::write(os, v); }
        ...
        std::size_t serializer<std::chrono::system_clock::time_point, TYPED>::write(
            std::ostream &os,
            ...
            os << '"' << std::put_time(&tm, "%Y-%m-%dT%H:%M:%S") << '"' << type.type_name();
            return cs;
        }
