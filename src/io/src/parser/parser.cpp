...
namespace cyng {
    namespace io {
        ...
        void parser::put(char c) {
            ...
            case state::FAILURE: BOOST_ASSERT(state_ != state::FAILURE); break;

            case state::TYPE: state_ = type(c); break;
            case state::LENGTH: state_ = length(c); break;
            case state::VALUE: state_ = value(c); break;
        ...
        object parser::deserialize() {
            switch (type_) {
            case TC_NULL: BOOST_ASSERT(buffer_.empty()); return make_object();

            case TC_BOOL: BOOST_ASSERT(buffer_.size() == 1); return make_object(buffer_.at(0) == 1);
            case TC_CHAR: BOOST_ASSERT(buffer_.size() == 1); return make_object(buffer_.at(0));
...
//            case TC_STRING_U8: return make_object(make_string<std::u8string::value_type>(buffer_));
            ...
            case TC_MAC48: BOOST_ASSERT(buffer_.size() == mac48::size()); return make_object(make_mac48(buffer_));
            case TC_MAC64: BOOST_ASSERT(buffer_.size() == mac64::size()); return make_object(make_mac64(buffer_));
            case TC_PID: return make_object(make_pid(buffer_));

            case TC_OBIS: BOOST_ASSERT(buffer_.size() == obis::size()); return make_object(make_obis(buffer_));
            case TC_OBISPATH: BOOST_ASSERT((buffer_.size() % obis::size()) == 0); return make_object(make_obis_path(buffer_));
            case TC_EDIS: BOOST_ASSERT(buffer_.size() == edis::size()); return make_object(make_edis(buffer_));
            case TC_COLOR_8: BOOST_ASSERT(buffer_.size() == sizeof(color_8::rgb_type)); return make_object(make_color8(buffer_));
            case TC_COLOR_16: BOOST_ASSERT(buffer_.size() == sizeof(color_16::rgb_type)); return make_object(make_color16(buffer_));
            ...
            case TC_IP_TCP_ENDPOINT:
                return make_object(make_endpoint<traits::reverse_type<TC_IP_TCP_ENDPOINT>::type::protocol_type>(buffer_));
            case TC_IP_UDP_ENDPOINT:
                return make_object(make_endpoint<traits::reverse_type<TC_IP_UDP_ENDPOINT>::type::protocol_type>(buffer_));
            case TC_IP_ICMP_ENDPOINT:
                return make_object(make_endpoint<traits::reverse_type<TC_IP_ICMP_ENDPOINT>::type::protocol_type>(buffer_));

            case TC_EOD: BOOST_ASSERT(buffer_.empty()); return make_eod();
