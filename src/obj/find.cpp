...
namespace cyng {
	...
	namespace {
		 ...
		 */
		template <typename T >
		auto search(T const* p, std::size_t idx) -> typename T::const_iterator
		{
			return std::find_if(std::begin(*p), std::end(*p), [idx](object const& obj) -> bool {
				...
				case TC_UINT32:
					return *object_cast<std::uint32_t>(obj) == static_cast<std::uint32_t>(idx);
				case TC_UINT64:
					return *object_cast<std::uint64_t>(obj) == static_cast<std::uint64_t>(idx);
				default:
					break;
				}
				return false;
		  ...
		  */
		template <typename T >
		auto search(T const* p, std::string const& name) -> typename T::const_iterator
		{
			return std::find_if(std::begin(*p), std::end(*p), [&name](object const& obj) -> bool {
				...
				case TC_STRING:
					return boost::algorithm::equals(*object_cast<std::string>(obj), name);
				default:
					break;
				}
				return false;
		...
		{
			return (boost::algorithm::equals(p->first, name))
				? p->second
				: make_object()
				;
		}
	...
	object find(deque_t const& v, std::size_t idx)
	{
		return find(&v, idx);
	}
	...
	object find(object const& obj, std::size_t idx)
	{
		...
		case TC_DEQUE:
			return find(object_cast<deque_t>(obj), idx);
		case TC_ATTR_MAP:
	...
	object find(deque_t const& v, std::string const& name)
	{
		return find(&v, name);
	}
	...
	object find(object const& obj, std::string const& name)
	{
		...
		case TC_DEQUE:
			return find(object_cast<deque_t>(obj), name);
		case TC_PARAM_MAP:
