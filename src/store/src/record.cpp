...
namespace cyng {
    ...
    bool record::empty() const { return !meta_.check_integrity(key_, data_); }

    key_t const &record::key() const { return key_; }

    /**
     * @return the data part of the record.
     */
    data_t const &record::data() const { return data_; }

    /*
     * @return generation counter
     */
    std::uint64_t record::get_generation() const { return gen_; }
    ...
    tuple_t record::to_tuple(param_map_t const &pm) const {
        ...
        //
        std::for_each(pm.begin(), pm.end(), [&data](param_map_value const &val) { data[val.first] = val.second; });

        return cyng::make_tuple(make_param("key", key), make_param("data", data));
    }
    ...
    object record::get_data(std::size_t idx) const { return (idx < data_.size()) ? data_.at(idx) : object(); }
    ...
    object record::at(std::size_t idx) const {
        if (idx < meta_.size()) {
            return (meta_.is_pk(idx)) ? key_.at(idx) : get_data(idx - meta_.key_size());
        }
        return object();
    }
} // namespace cyng
