...
namespace cyng {
    namespace sql {
        ...
        std::string variable<bool>::to_string(dialect d, meta_sql const &) const {
            return (d == dialect::SQLITE) ? (b_ ? "1" : "0") : (b_ ? "TRUE" : "FALSE");
        }
        ...
        std::string variable<std::chrono::system_clock::time_point>::to_string(dialect d, meta_sql const &) const {
            ...
            std::stringstream ss;
            if (dialect::SQLITE == d) {
                ss << "julianday('" << std::put_time(&tm, "%F %T") << "')";
            } else {
         ...
         * date
         */
        variable<date>::variable(date &tp)
            : tp_(tp) {}

        std::string variable<date>::to_string(dialect d, meta_sql const &) const {
            std::stringstream ss;
            if (dialect::SQLITE == d) {
                ss << "julianday('" << as_string(tp_, "%F %T") << "')";
            } else {
                ss << as_string(tp_, "%F %T");
            }
            return ss.str();
        }
