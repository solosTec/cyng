...
namespace cyng 
{
	...
	bool test_store_001()
	{
		auto body =  store::body_generator(1, 2, 3);
		
		
		store::meta_table<2, 5> mt1("table-1");
		store::meta_table<2, 5> mt2("table-2", {"key-0", "key-1", "body-0", "body-1", "body-2", "body-3", "body-4"});
		
// 		std::cout << mt2.get_body_index("body-3").first << std::endl;
		BOOST_CHECK_EQUAL(mt2.get_body_index("body-3").first, 3);
		
		auto mtp = store::make_meta_table<2, 5>("table-2", {"key-0", "key-1", "body-0", "body-1", "body-2", "body-3", "body-4"}, {});
// 		std::cout << mtp->get_body_index("body-4").first << std::endl;
		...
		tbl_1.insert(store::key_generator(1, 2), store::data_generator(1, 2, 3, 4, 5), 0);
...
// 		std::cout << tbl_1.size() << std::endl;
		...
		tbl_1.erase(store::key_generator(1, 2));
//  		std::cout << tbl_1.size() << std::endl;
...
//  				std::cout << "INS: " << i1 << ", " << i2 << ", " << tbl_1.size() << ", " << (i1*upper_limit +i2) << std::endl;
				...
				tbl_1.insert(store::key_generator(i1, i2), store::data_generator(i1, i2, i1+i2, i1-i2, i2-i1), 0);
			}
		}
...
// 				tbl_1.insert(store::key_generator(i1, i2), store::data_generator(i1, i2, i1+i2, i1-i2, i2-i1));
				auto rec = tbl_1.lookup(store::key_generator(i1, i2));
