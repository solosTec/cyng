...
// 		std::this_thread::sleep_for(std::chrono::milliseconds(1));
	...
	bool test_store_002()
	{
		auto dbmtp = store::make_meta_table<1, 2>("db", {"name", "table", "created"});
...
// 		key_type const& key, data_type const& data
		const bool b = db_tbl.insert(store::key_generator("name"), store::data_generator(tbl_obj, std::chrono::system_clock::now()), 0);
		BOOST_CHECK(b);
		
		store::db db_1;
		db_1.create_table(store::make_meta_table<2, 5>("table-1", {"key-0", "key-1", "body-0", "body-1", "body-2", "body-3", "body-4"}));
		db_1.create_table(store::make_meta_table<2, 5>("table-2", {"key-0", "key-1", "body-0", "body-1", "body-2", "body-3", "body-4"}));
