...
namespace cyng 
{
	...
	{
		vm.async_run(register_function("ip.tcp.socket.shutdown", 0, [&s](context& ctx) {
		...
		//
		vm.async_run(register_function("ip.tcp.socket.close", 0, [&s](context& ctx) {
			boost::system::error_code ec;
			s.close(ec);
			ctx.set_register(ec);
		}));
		...
		//
		vm.async_run(register_function("ip.tcp.socket.cancel", 0, [&s](context& ctx) {
			boost::system::error_code ec;
			s.cancel(ec);
			ctx.set_register(ec);
		}));
		...
		//	the boost::asio::error::operation_aborted error.
		//
		vm.async_run(register_function("ip.tcp.socket.isOpen", 0, [&s](context& ctx) {
		...
		//
		vm.async_run(register_function("ip.tcp.socket.localEndpoint", 0, [&s](context& ctx) {
			boost::system::error_code ec;
		...
		//
		vm.async_run(register_function("ip.tcp.socket.remoteEndpoint", 0, [&s](context& ctx) {
			boost::system::error_code ec;
		...
		//
		vm.async_run(register_function("ip.tcp.socket.connect", 1, [&s](context& ctx) {
		...
		//
		vm.async_run(register_function("ip.tcp.socket.available", 0, [&s](context& ctx) {
			boost::system::error_code ec;
		...
		//
		vm.async_run(register_function("ip.tcp.socket.write", 1, [&s](context& ctx) {
			const vector_t frame = ctx.get_frame();
			boost::system::error_code ec;
			buffer_t buf;
