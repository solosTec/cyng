...
BOOST_AUTO_TEST_CASE(vm) {
    ...
    vm.execute(cyng::op::NOW, cyng::make_object("abc"), cyng::op::MAKE_PARAM, cyng::TC_PARAM, cyng::op::ASSERT_TYPE);
...
class session {
  public:
    session() {}
    inline std::uint64_t foo(std::string s) const { return s.size(); }
    inline std::uint64_t bar(std::string s) { return s.size(); }
};
...
class sig {
    ...
    using signatures_t = std::tuple<std::function<void(std::string)>, std::function<void(cyng::eod)>>;
