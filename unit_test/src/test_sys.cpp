...
BOOST_AUTO_TEST_CASE(meminfo) {
    ...
    auto constexpr tr2 = cyng::sys::get_host_total_physical_memory(); //	MB
...
BOOST_AUTO_TEST_CASE(net) {
    ...
    // cyng::sys::read_ipv6_info([](std::string address, std::string name, std::uint64_t index, std::uint64_t len, std::uint64_t
    // scope, std::uint64_t flag) -> bool { 	std::cout << address << " - " << name << " - " << scope << std::endl; 	if
    // (boost::algorithm::equals(name, "ens33") && 0x020 == scope) { 		std::cout << cyng::to_ipv6(address, 0) << "%" <<
    // name
...
//         auto const ts = cyng::sys::to_time_point(inp, fmt);
...
//     // cyng::sys::to_string(std::cout, v0, "%Y-%m-%dT%H:%M%z\n");
//     // cyng::sys::to_string_utc(std::cout, v0, "%Y-%m-%dT%H:%M%z\n");
//
//     {
//         /*
//
//         std::istringstream ss("2022-September-10 16:56:32");
...
//     std::endl; std::cout << "__cpp_lib_addressof_constexpr\t: " << __cpp_lib_addressof_constexpr << std::endl; std::cout <<
//     "__cpp_lib_any\t\t\t: " << __cpp_lib_any << std::endl; std::cout << "__cpp_lib_apply\t\t\t: " << __cpp_lib_apply <<
//     std::endl;
// }
