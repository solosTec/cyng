...
BOOST_AUTO_TEST_CASE(parser) {
    ...
    obj = cyng::make_object(
        boost::uuids::uuid{{0x7c, 0xa6, 0xb0, 0x5c, 0x2a, 0x02, 0x47, 0x55, 0x9e, 0xc5, 0xe6, 0xd0, 0x6a, 0x45, 0x89, 0xdd}});
    ...
    //
    obj = cyng::make_object(
        boost::asio::ip::tcp::endpoint(boost::asio::ip::make_address("[2001:171b:226b:a7d0:225:18ff:fea4:9982]"), 7001u));
    cmp = cyng::io::to_typed(obj); //
    inp = convert(obj);
    p.read(std::begin(inp), std::end(inp));
    ...
    //
    obj = cyng::make_object(
        cyng::obis_path_t{cyng::obis(0x01, 0x00, 0x00, 0x00, 0x09, 0xff), cyng::obis(0x01, 0x00, 0x00, 0x00, 0x09, 0x01)});
    cmp = cyng::io::to_typed(obj); //
    inp = convert(obj);
    p.read(std::begin(inp), std::end(inp));
    ...
    //	date
    //
    obj = cyng::make_object(cyng::date(2022, 7, 10, 16, 56, 32));
    cmp = cyng::io::to_typed(obj); //
    inp = convert(obj);
    p.read(std::begin(inp), std::end(inp));
